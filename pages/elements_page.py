...
class CheckboxPage(BasePage):
    ...
    def open_full_list(self):
        btn = self.element_is_visible(self.locators.EXPAND_ALL_BUTTON).click()
    ...
    def get_checked_checkboxes(self):
        ...
        for box in checked_list:
            title_item = box.find_element(By.XPATH, self.locators.TITLE_ITEM).text
            title_item = title_item.replace(' ', '')
            title_item = title_item.replace('.doc', '')
            title_item = title_item.lower()
...
class WebTablePage(BasePage):
    ...
    def select_up_to_some_rows(self):
        ...
        for x in count:
            ...
            self.goto_element(count_row_button)
            count_row_button.click()
            self.element_is_visible(By.CSS_SELECTOR, 'option[value="{}"]'.format(x)).click()
...
class UploadAndDownloadPage(BasePage):
    ...
    def upload_file(self):
        ...
        # file_local_name = path.split('\\')[-1]
        file_local_name = file_name.split('\\')[-1]
        file_uploaded = text.split('\\')[-1]
        return file_local_name, file_uploaded
        #assert file_local_name == file_uploaded

    def download_file(self):
        link = self.element_is_present(self.locators.DOWNLOAD_FILE).get_attribute('href')
        link_b = base64.b64decode(link)
        path_name_file = rf'C:\Files\filetst{random.randint(0, 999)}.jpeg'
